...
if [ ! -z "${TG_TOKEN}"];  then TG_TOKEN=${TG_TOKEN}; else TG_TOKEN=$(cat $(find /home/opt -iname ".tg_token")); fi
if [ ! -z "${CHAT_ID}" ]; then CHAT_ID=${CHAT_ID}; else CHAT_ID=$(cat $(find /home/opt/ -iname ".chat_id")); fi
if [ ! -z "${M_ID}" ]; then M_ID=${M_ID}; else M_ID=$(cat $(find /home/opt/ -iname ".m_id")); fi
if [ ! -z "${C_ID}" ]; then C_ID=${C_ID}; else C_ID=$(cat $(find /home/opt/ -iname ".c_id")); fi
GIT_TKN=$(cat $(find /home/runner/work/DumprX-Workflow/DumprX-Workflow/phoenix_firmware_dumper -iname ".github_token"))
if [ ! -z "${MESSAGE_ID}" ]; then MESSAGE_ID=${MESSAGE_ID}; else MESSAGE_ID=$(cat $(find /home/opt/ -iname ".message_id")); fi

if [ ${#GIT_TKN} -lt 40 ]; then echo "Invalid Github Token"; fi
if [ ! -z "${TG_TOKEN}" ]; then
    [[ "$VERBOSE" != "n" ]] && echo -e "Sending telegram notification"
else
    echo "Tg_token is missing"
fi
if [ ! -z "${GIT_TKN}" ]; then
    [[ "$VERBOSE" != "n" ]] && echo -e "Git token found"
else
...
call_methods() {
    ...
    # create repo's
    if [[ ! -z "$GIT_TKN" ]]; then
        if [ ${#GIT_TKN} == 40 ]; then echo "Github Token is correct" ; fi
        ORG="something6969"
        #GITHUB_EMAIL="${ORG}@github.com"
        curl -s -X POST -H "Authorization: token ${GIT_TKN}" -d '{"name": "'"$DT_REPO"'","description": "'"$DT_REPO_DESC"'","private": false,"has_issues": true,"has_projects": false,"has_wiki": true}' "https://api.github.com/orgs/$ORG/repos" 
        curl -s -X POST -H "Authorization: token ${GIT_TKN}" -d '{"name": "'"$VT_REPO"'","description": "'"$VT_REPO_DESC"'","private": false,"has_issues": true,"has_projects": false,"has_wiki": true}' "https://api.github.com/orgs/$ORG/repos"
        curl -s -X PUT -H "Authorization: token ${GIT_TKN}" -H "Accept:bcation/vnd.github.mercy-preview+json" -d '{ "names": ["'"$TOPIC1"'","'"$TOPIC2"'","'"$TOPIC3"'"]}' "https://api.github.com/repos/${ORG}/${VT_REPO}/topics"
    elif [[ ! -z "$GIT_TKN" ]] && [[ ! -z "$GITHUB_EMAIL" ]] && [[ ! -z "$GITHUB_USER" ]]; then
        ORG="$GITHUB_USER"
...
git_op() {
    ...
    if [ ! -z "${TG_TOKEN}" ]; then
        ...
        printf "\n<b>Follow @saurajdumps</b>" >> /home/runner/work/dumper/dumper/telegram/tg.html
        CHAT_ID="@saurajdumps"
